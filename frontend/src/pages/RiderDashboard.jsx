...
export default function RiderDashboard() {
  ...
  async function book(matchTripId) {
    ...
    setMatchInfo(null);
    if (myTrips.length === 0) {
      setError("Create a rider trip first.");
      return;
    }
  ...
  return (
    <div>
      ...
      <TripList trips={myTrips} title="My rider trips" />
      <div style={{ marginTop: 24 }}>
        <h4>Find drivers with same destination</h4>
        <input
        ...
        {matches.map((m) => (
          ...
          >
            <div>
              Driver: {m.user_name} | {m.origin} → {m.destination}
            </div>
            <div>Time: {m.time}</div>
            <div>Price: {m.price}</div>
            <button onClick={() => book(m.id)}>Book this driver</button>
          </div>
        ))}
      </div>
